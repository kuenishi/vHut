...
package jp.co.ntts.vhut.entity {
    ...
    [Bindable]
    [RemoteClass(alias="jp.co.ntts.vhut.entity.ApplicationVm")]
	/**
	 * ApplicationVm Entity Class.
	 * <p></p>
	 *
	 * <p>
	 ...
	 */
    public class ApplicationVm extends ApplicationVmBase implements IClonable, IVaElement
	{
		public static const UPDATE_LINK_LIST:String = "updateLinkList";

		public function ApplicationVm()
		{
			super();
			name = "New Vm"
			description = "";
		...
		override public function set applicationVmSecurityGroupMapList(value:ArrayCollection):void {
			...
			{
				if(item.applicationVmId == id)
				{
					item.applicationVm = this;
				}
			}
		}
		...
		public function clone(withId:Boolean = true, deep:Boolean = true, ref:Dictionary = null):*
		{
			...
			{
				var result:ApplicationVm = ref[this] as ApplicationVm;
				if(result != null)
				{
					return result;
				}
			}
			result = new ApplicationVm();
			CloneUtil.updateProperties(result, this, "jp.co.ntts.vhut.entity.ApplicationVmBase");
			ref[this] = result;
		...
		{
			var indexInAvm:int = applicationVmSecurityGroupMapList.getItemIndex(map);
			if(indexInAvm == -1) return null;

			var indexInAsg:int = map.applicationSecurityGroup.applicationVmSecurityGroupMapList.getItemIndex(map);
			if(indexInAsg != -1) map.applicationSecurityGroup.applicationVmSecurityGroupMapList.removeItemAt(indexInAsg);

			applicationVmSecurityGroupMapList.removeItemAt(indexInAvm);

			return map;
		}

		/**
		 * ApplicationSecurityGroupと接続を持つApplicationVmSecurityGroupMapを取得します.
		 * @param sg ApplicationSecurityGroup
		 * @return ApplicationVmSecurityGroupMap
		 */
		public function getApplicationVmSecurityGroupMapWithConnectionTo(asg:ApplicationSecurityGroup):ApplicationVmSecurityGroupMap
		{
			for each(var map:ApplicationVmSecurityGroupMap in applicationVmSecurityGroupMapList)
			{
				if(map.applicationSecurityGroup == asg) return map;
			}
			return null;
		}
		...
		/**
		 * ApplicationSecurityGroupとの接続を切断します.
		 * @param asg 切断するApplicationSecurityGroup
		 * @return 削除されたMap
		 */
		public function disconnectTo(asg:ApplicationSecurityGroup):ApplicationVmSecurityGroupMap
		{
			var map:ApplicationVmSecurityGroupMap = getApplicationVmSecurityGroupMapWithConnectionTo(asg);
			var index:int = applicationVmSecurityGroupMapList.getItemIndex(map);
		...
		public function get pos():Point
		{
			...
			return application.topology.getVmPos(privateId);
		}
		public function set pos(value:Point):void
		{
			if(application != null)
				application.topology.setVmPos(privateId, value);
		}
