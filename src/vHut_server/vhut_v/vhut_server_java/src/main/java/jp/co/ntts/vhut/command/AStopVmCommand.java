 ...
 * -->
 */
public class AStopVmCommand extends AbstractAsyncCloudDriverCommand {
    private static final VhutLogger logger = VhutLogger.getLogger(AStopVmCommand.class);
     ...
     * デフォルトコンストラクタ.
     */
    public AStopVmCommand() {
        operation = CommandOperation.STOP_VM_ASYNC;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            if (isPrivate) {
                //プライベートクラウドの処理
                prcDirver.stopVmAsync((Long) parameter.get(Key.VM_ID), command.id);
            } else {
                //パブリッククラウドの処理
            }
