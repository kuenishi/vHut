...
/**
 * <p>ネットワーク管理サーバに問い合わせて、DHCPルールに指定したIP+MACに関するルールを追加します.
 * <br>
 * <p>{@link NwDriver#addIp(String, String)}に依存しています.
 *
 * @version 1.0.0
 ...
 * -->
 */
public class SAddIpCommand extends AbstractNetworkDriverCommand {
    private static final VhutLogger logger = VhutLogger.getLogger(SAddIpCommand.class);
     ...
     * フレームワークから呼び出されます
     */
    public SAddIpCommand() {
        operation = CommandOperation.ADD_IP;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            result = nwDriver.addIp((String) parameter.get(Key.IP), (String) parameter.get(Key.MAC));
