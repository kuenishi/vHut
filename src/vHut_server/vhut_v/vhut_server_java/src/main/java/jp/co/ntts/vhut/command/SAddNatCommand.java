...
/**
 * <p>ネットワーク管理サーバに問い合わせて、iptablesルールに指定したIPに関連したルールを追加します.
 * <br>
 * <p>{@link NwDriver#addNat(String, String)}に依存しています.
 *
 * @version 1.0.0
 ...
 * -->
 */
public class SAddNatCommand extends AbstractNetworkDriverCommand {
    private static final VhutLogger logger = VhutLogger.getLogger(SAddNatCommand.class);
     ...
     * フレームワークから呼び出されます
     */
    public SAddNatCommand() {
        operation = CommandOperation.ADD_NAT;
    }
     ...
     * @param publicIp 外部IPアドレス
     * @param privateIp 内部IPアドレス
     */
    ...
    public CommandStatus execute() {
        ...
        try {
            result = nwDriver.addNat((String) parameter.get(Key.PUBLIC_IP), (String) parameter.get(Key.PRIVATE_IP));
