 ...
 * -->
 */
public class SGetAllClustersCommand extends AbstractCloudDriverCommand {
     ...
     * デフォルトコンストラクタ.
     */
    public SGetAllClustersCommand() {
        operation = CommandOperation.GET_ALL_CLUSTERS;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            if (isPrivate) {
                // プライベートクラウドの処理
                result = new ArrayList<Cluster>(prcDirver.getAllClusters());
            } else {
                // パブリッククラウドの処理
            }
    ...
    public void finish() {
        try {
            updateSynchronizedEntiteis(Cluster.class);
        } catch (Exception e) {
