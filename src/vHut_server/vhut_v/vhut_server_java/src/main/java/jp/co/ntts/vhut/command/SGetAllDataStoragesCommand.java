 ...
 * -->
 */
public class SGetAllDataStoragesCommand extends AbstractCloudDriverCommand {

    private static final VhutLogger logger = VhutLogger.getLogger(SGetAllDataStoragesCommand.class);


    /**
     * デフォルトコンストラクタ.
     */
    public SGetAllDataStoragesCommand() {
        operation = CommandOperation.GET_ALL_DATA_STORAGES;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            if (isPrivate) {
                // プライベートクラウドの処理
                result = new ArrayList<Storage>(prcDirver.getAllDataStorages());
            } else {
                // パブリッククラウドの処理
            }
    ...
    public void finish() {
        try {
            //エンティティの同期
            updateSynchronizedEntiteis(Storage.class);
            //リソースの更新
            cloudInfraLogic.updateStorageResource();
        } catch (Exception e) {
