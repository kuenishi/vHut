...
/**
 * <p>指定したクラスターに関連するホストの一覧を取得するコマンド.
 * <br>
 *
 * @version 1.0.0
 ...
 * -->
 */
public class SGetHostsByClusterIdCommand extends AbstractCloudDriverCommand {

    private static final VhutLogger logger = VhutLogger.getLogger(SGetHostsByClusterIdCommand.class);
     ...
     * デフォルトコンストラクタ.
     */
    public SGetHostsByClusterIdCommand() {
        operation = CommandOperation.GET_HOSTS_BY_CLUSTER_ID;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            if (isPrivate) {
                // プライベートクラウドの処理
                result = new ArrayList<Host>(prcDirver.getHostsByClusterId((Long) parameter.get(Key.CLUSTER_ID)));
            } else {
                // パブリッククラウドの処理
            }
    ...
    public void finish() {
        try {
            //エンティティの同期
            updateSynchronizedEntiteis(Host.class);
            //リソースの更新
            cloudInfraLogic.updateClusterResource();
        } catch (Exception e) {
