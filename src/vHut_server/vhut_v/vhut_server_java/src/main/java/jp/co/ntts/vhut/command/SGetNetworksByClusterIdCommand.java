...
/**
 * <p>指定したクラスターに関連するネットワークの一覧を取得するコマンド.
 *
 * @version 1.0.0
 ...
 * -->
 */
public class SGetNetworksByClusterIdCommand extends AbstractCloudDriverCommand {

    private static final VhutLogger logger = VhutLogger.getLogger(SGetNetworksByClusterIdCommand.class);
     ...
     * デフォルトコンストラクタ.
     */
    public SGetNetworksByClusterIdCommand() {
        operation = CommandOperation.GET_NETWORKS_BY_CLUSTER_ID;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            if (isPrivate) {
                // プライベートクラウドの処理
                result = new ArrayList<Network>(prcDirver.getNetworksByClusterId((Long) parameter.get(Key.CLUSTER_ID)));
    ...
    public void finish() {
        try {
            updateSynchronizedEntiteis(Network.class);
        } catch (Exception e) {
