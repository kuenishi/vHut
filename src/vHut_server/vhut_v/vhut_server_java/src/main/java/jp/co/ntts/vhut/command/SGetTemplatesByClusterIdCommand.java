...
/**
 * <p>指定したクラスターに関連するテンプレートの一覧を取得するコマンド.
 * <br>
 *
 * @version 1.0.0
 ...
 * -->
 */
public class SGetTemplatesByClusterIdCommand extends AbstractCloudDriverCommand {

    private static final VhutLogger logger = VhutLogger.getLogger(SGetTemplatesByClusterIdCommand.class);
     ...
     * デフォルトコンストラクタ.
     */
    public SGetTemplatesByClusterIdCommand() {
        operation = CommandOperation.GET_TEMPLATES_BY_CLUSTER_ID;
    }
    ...
    public CommandStatus execute() {
        ...
        try {
            if (isPrivate) {
                // プライベートクラウドの処理
                result = new ArrayList<Template>(prcDirver.getTemplatesByClusterId((Long) parameter.get(Key.CLUSTER_ID)));
            } else {
                // パブリッククラウドの処理
            }
    ...
    public void finish() {
        try {
            updateSynchronizedEntiteis(Template.class);
        } catch (Exception e) {
