/*
 * Copyright 2011 NTT Software Corporation.
 * All Rights Reserved.
 */
package jp.co.ntts.vhut.entity;

import java.io.Serializable;
import java.util.ArrayList;
import java.util.List;

import javax.annotation.Generated;
import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.EnumType;
import javax.persistence.Enumerated;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.ManyToOne;
import javax.persistence.OneToMany;
import javax.persistence.OneToOne;
import javax.persistence.Transient;
import javax.persistence.Version;

import jp.co.ntts.vhut.exception.DBConflictException;

/**
 * Networkエンティティクラス.
 *
 * @author NTT Software Corporation.
 * @version 1.0.0
 * <p>
 * <!--$Date: 2011-11-28 19:50:40 +0900 (月, 28 11 2011) $<br>-->
 * <!--$Revision: 949 $<br>-->
 * <!--$Author: NTT Software Corporation. $<br>-->
 */
@Entity
@Generated(value = { "S2JDBC-Gen 2.4.41", "org.seasar.extension.jdbc.gen.internal.model.EntityModelFactoryImpl" }, date = "2011/07/15 15:30:33")
public class Network implements Serializable, ISynchronizedEntity {

    private static final long serialVersionUID = 1L;

    /** idプロパティ */
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(precision = 19, nullable = false, unique = true)
    public Long id;

    /** cloudIdプロパティ */
    @Column(precision = 19, nullable = false, unique = false)
    public Long cloudId;

    /** nameプロパティ */
    @Column(length = 256, nullable = true, unique = false)
    public String name;

    /** vlanプロパティ
     * VLAN番号
     */
    @Column(precision = 5, nullable = true, unique = false)
    public Short vlan;

    /** networkAddressプロパティ */
    @Column(length = 8, nullable = true, unique = false)
    public String networkAddress;

    /** maskプロパティ */
    @Column(length = 8, nullable = true, unique = false)
    public String mask;

    /** gatewayプロパティ */
    @Column(length = 8, nullable = true, unique = false)
    public String gateway;

    /** dnsプロパティ */
    @Column(length = 8, nullable = true, unique = false)
    public String dns;

    /** broadcastプロパティ */
    @Column(length = 8, nullable = true, unique = false)
    public String broadcast;

    /** dhcpプロパティ */
    @Column(length = 8, nullable = true, unique = false)
    public String dhcp;

    /** statusプロパティ */
    @Enumerated(EnumType.ORDINAL)
    public NetworkStatus status;

    /** conflictIdプロパティ */
    @Column(precision = 19, nullable = true, unique = false)
    public Long conflictId;

    /** versionプロパティ */
    @Version
    @Column(precision = 19, nullable = false, unique = false, columnDefinition = "default 0")
    public Long version = 0L;

    /** conflict関連プロパティ */
    @ManyToOne
    public Conflict conflict;

    /** securityGroup関連プロパティ */
    @OneToOne(mappedBy = "network")
    public SecurityGroup securityGroup;

    /** vlanReservationList関連プロパティ */
    @OneToMany(mappedBy = "network")
    public List<VlanReservation> vlanReservationList;

    @Transient
    private List<Object> updateChildren;


    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#getConflict()
     */
    @Override
    public Conflict getConflict() {
        return conflict;
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#getConflictId()
     */
    @Override
    public Long getConflictId() {
        return conflictId;
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#getId()
     */
    @Override
    public Long getId() {
        return id;
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#getCloudId()
     */
    @Override
    public Long getCloudId() {
        return cloudId;
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#setConflict(jp.co.ntts.vhut.entity.Conflict)
     */
    @Override
    public void setConflict(Conflict value) {
        conflict = value;
        conflictId = conflict.id;
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#setConflictId(java.lang.Long)
     */
    @Override
    public void setConflictId(Long value) {
        conflictId = value;
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#sync(jp.co.ntts.vhut.exception.ISynchronizedEntity)
     */
    @Override
    public boolean sync(ISynchronizedEntity entity) throws DBConflictException {
        Network remote = (Network) entity;
        boolean isUpdated = false;

        DBConflictException exception = new DBConflictException(this);

        // 名称
        if (!name.equals(remote.name)) {
            exception.addChanged("natwork.name", name, remote.name);
        }

        // VLAN
        if (!vlan.equals(remote.vlan)) {
            exception.addChanged("network.vlan", vlan, remote.vlan);
        }

        // ネットワークアドレス
        if (!networkAddress.equals(remote.networkAddress)) {
            exception.addChanged("network.networkAddress", networkAddress, remote.networkAddress);
        }

        // DNS
        if (!dns.equals(remote.dns)) {
            exception.addChanged("network.dns", dns, remote.dns);
        }

        //        // ブロードキャスト
        //        if (!broadcast.equals(remote.broadcast)) {
        //            exception.addChanged("network.broadcast", broadcast, remote.dns);
        //        }

        // サブネット
        if (!mask.equals(remote.mask)) {
            exception.addChanged("network.mask", mask, remote.mask);
        }

        // ゲートウェイ
        if (!gateway.equals(remote.gateway)) {
            exception.addChanged("network.geteway", gateway, remote.gateway);
        }

        //状態
        if (!status.equals(remote.status)) {
            status = remote.status;
            isUpdated = true;
        }

        if (exception.getSize() > 0) {
            // 不一致を検知
            throw exception;
        } else {
            // 一致
            return isUpdated;
        }
    }

    /* (non-Javadoc)
     * @see jp.co.ntts.vhut.exception.ISynchronizedEntity#getUpdatedChildren()
     */
    @Override
    public List<Object> getUpdatedChildren() {
        if (updateChildren == null) {
            updateChildren = new ArrayList<Object>();
        }
        return updateChildren;
    }
}


/**
 * =====================================================================
 * 
 *    Copyright 2011 NTT Sofware Corporation
 * 
 *    Licensed under the Apache License, Version 2.0 (the "License");
 *    you may not use this file except in compliance with the License.
 *    You may obtain a copy of the License at
 * 
 *        http://www.apache.org/licenses/LICENSE-2.0
 * 
 *    Unless required by applicable law or agreed to in writing, software
 *    distributed under the License is distributed on an "AS IS" BASIS,
 *    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *    See the License for the specific language governing permissions and
 *    limitations under the License.
 * 
 * =====================================================================
 */
